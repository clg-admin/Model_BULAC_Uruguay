...
# Function #28
def project_demand(time_vector, ini_simu_yr, projtype, mask, df, sum_demand_by, gdp_growth_vals, gdp_growth_vals_2, gdp_pc_growth_vals, gdp_pc_growth_vals_2, pop_growth_vals, ela_list, ela_list_2, type):
	"""
	Project transport demand based on the specified projection type and growth parameters.

	Args:
	- time_vector (list): List of years for which the projection is calculated.
	- ini_simu_yr (int): Initial simulation year to determine projection applicability.
	- projtype (str): Projection method ('endogenous_gdp', 'endogenous_gdp_pc', 'endogenous_pop').
	- mask (pandas.core.series.Series): Mask or filter criteria applied to the data.
	- df (pandas.DataFrame): DataFrame containing relevant data for projection.
	- sum_demand_by (float): Initial total demand used as a baseline for projections.
	- gdp_growth_vals (list[type]): List of annual GDP growth rates (in percentage).
	- gdp_growth_vals_2 (list[type]): Alternative GDP growth rates for dual scenarios (in percentage).
	- gdp_pc_growth_vals (list[type]): List of annual GDP per capita growth rates (in percentage).
	- gdp_pc_growth_vals_2 (list[type]): Alternative GDP per capita growth rates for dual scenarios (in percentage).
	- pop_growth_vals (list[type]): List of annual population growth rates (in percentage).
	- ela_list (list[type]): List of elasticity values corresponding to the time vector.
	- ela_list_2 (list[type]): Alternative elasticity values for dual scenarios.
	- type (str): Type of demand ('pas' for passenger or 'fre' for freight).

	Returns:
	- trn_dem (list): Projected transport demand values for each year in the time vector.
	- trn_dem_2 (list): Alternative projected transport demand values considering dual scenarios.
	"""
	trn_dem = [0 for y in range(len(time_vector))]
	trn_dem_2 = [0 for y in range(len(time_vector))]
	for y in range(len(time_vector)):
		if y == 0:
			trn_dem[y] = sum_demand_by
		else:
			gdp_gr = gdp_growth_vals[y] / 100
			gdp_gr_2 = gdp_growth_vals_2[y]/100
			gdp_pc_gr = gdp_pc_growth_vals[y] / 100
			gdp_pc_gr_2 = gdp_pc_growth_vals_2[y]/100
			...
			if projtype == 'endogenous_gdp':
				if type=='pas':
					...
					trn_gr = 1 + (gdp_gr * ela_list[y])
					trn_gr_2 = 1 + (gdp_gr_2*ela_list_2[y])
			elif projtype == 'endogenous_gdp_pc':
				if type=='pas':
					trn_gr = 1 + (gdp_pc_gr * ela_list[y])
					trn_gr_2 = 1 + (gdp_pc_gr_2*ela_list_2[y])
				elif type=='fre':
					trn_gr = 1 + (gdp_pc_gr * ela_list[y])
			...
			if 'endogenous' in projtype:
				...
				if time_vector[y] > ini_simu_yr:
					trn_dem_2[y] = trn_gr_2*trn_dem_2[y-1]
				else:
					trn_dem_2[y] = trn_gr*trn_dem[y-1]
				
				
	return trn_dem, trn_dem_2

# # Function #28
# def project_demand(time_vector, ini_simu_yr, projtype, mask, df, sum_demand_by, gdp_growth_vals,gdp_pc_growth_vals, pop_growth_vals, ela_list, type):
# 	"""
# 	Project transport demand based on the specified projection type and growth parameters.
...
# Function #32
def extract_data_from_dataframe(condition, time_vector, dataframe, this_country):
	"""
	...
	return [dataframe.loc[mask][year].iloc[0] for year in time_vector], mask
...
# Function #34
def calculate_gpkm_for_types(time_vector, demand, priv_adj_share, pub_adj_share, pass_trn_dem_sh_private, \
				pass_trn_dem_sh_public, mode_shift, non_motorized, mask_mode_shift, mask_non_motorized, \
            priv_types, pub_types, val_mshft, df_trn_data, ini_simu_yr):
	"""
	...
	list_mshft = interpolation_non_linear_final(
		time_vector, list_mshft_raw, val_mshft, ini_simu_yr)
...
def update_fuel_economy_and_fleet(df, fuels, fuels_nonelectric, sh_non_electric_k, list_electrification, list_hydrogen, this_country, t, time_vector, val_elec, ini_simu_yr):
	...
	Args:
	- df (DataFrame): DataFrame containing transportation data for the analysis.
	- fuels (list[type]): List of all fuel types.
	- fuels_nonelectric (list[type]): List of non-electric fuel types.
	- sh_non_electric_k (dict[type]): Dictionary of shares for non-electric fuels.
	- list_electrification (list[type]): List of electrification shares for each year.
	- list_hydrogen (list[type]): List of hydrogen shares for each year.
	- this_country (str): Target country for data extraction and analysis.
	- t (str): Specific transport type being analyzed.
	- time_vector (list[type]): List of years for which the calculations are performed.
	- val_elec (float): Value used for non-linear interpolation of electrification data.
	- ini_simu_yr (int): Initial simulation year for the interpolation process.

	Returns:
	- list_fe_k (dict): Dictionary containing fuel economy values for each fuel type over the time vector.
	- list_nonele_fleet_k (dict): Dictionary containing non-electric fleet shares for each non-electric fuel type.
	"""
	list_non_electric = []
	list_fe_k = {}
	list_nonele_fleet_k = {}
	...
	list_electrification = \
		interpolation_non_linear_final(
			time_vector, list_elec_raw, val_elec, ini_simu_yr)

	list_h2_raw = deepcopy(list_hydrogen)
	list_hydrogen = \
		interpolation_non_linear_final(
			time_vector, list_h2_raw, val_elec, ini_simu_yr)

	for y in range(len(time_vector)):
		if list_electrification[y] + list_hydrogen[y] > 100:
			mult_ele_h2_norm = \
				100/(list_electrification[y] + list_hydrogen[y])
...
# Function #48
def apply_projection_overwrites(y, vehicle_type, fuel, projected_fleet_data, fleet, gpkm_gtkm, fuel_con, dict_km, fuel_efficiency, dict_lf, dict_fleet_k, dict_fuel_con, mult_fleet):
	"""
	...
	if projected_fleet_data[vehicle_type][fuel]['indicate'] == 'apply':
		...
		proj_fleet_y = projected_fleet_data[fuel][vehicle_type]['vals'][y]*mult_fleet[y]
...
def estimate_emissions(fuel_con, fuel, year_index, emissions_fuels_dict, dict_eq_transport_fuels, time_vector):
	...
	Args:
	- fuel_con (float): Fuel consumption for the specified fuel and year.
	- fuel (str): Type of fuel being analyzed.
	- year_index (int): Index of the year in the time vector.
	- emissions_fuels_dict (dict[type]): Dictionary containing emission factors for different fuels.
	  - Keys are fuel types, and values are lists of emission factors for each year.
	- dict_eq_transport_fuels (dict[type]): Dictionary mapping model-specific fuel types to those in the emission factor dictionary.
	- time_vector (list[type]): List of years for which the calculations are performed.

	Returns:
	- float: Estimated emissions for the specified fuel and year, calculated as `fuel_con * emission_factor`.
	- str: Equivalent fuel type in `emissions_fuels_dict` corresponding to the input fuel.
	"""
...
def calculate_emissions_externalities(demand_tech_list, dict_energy_demand, time_vector, \
										...
										emissions_demand_black_carbon, emissions_fuels_list, \
                                externality_fuels_list):
	"""
...
# Function #66
def create_loss_vector(df_param, time_vector, projection, baseyear_str, losses_share, ini_simu_yr):
	"""
...
def process_energy_distribution(df_param_related_4, tech_idx, time_vector, check_percent, energy_dist_1,\
                                this_country, base_year, ini_simu_yr, tech, store_percent, store_percent_rem):
	"""
...
# Function #91
def calculate_new_energy_assignment(y, tech, this_tech_base_cap, list_tech_cf, res_energy_sum_1, new_req_energy, store_percent, res_energy_sum, this_tech_dneeg_projection):
	"""
...
def manage_thermal_reductions(this_scen, tech, y, wtb_tech_list, thermal_reductions, 
							  ...
							  this_country,time_vector,store_res_energy,
...
def update_cf_for_thermal_gas(tech, y, new_energy_assign, res_energy, planned_energy, cf_by_tech, cf_ngas_max,
                              time_vector,list_tech_cf,store_res_energy,forced_newcap_energy_by_tech):
	"""
...
# # Function #112
# def calculate_generation(list_tech, total_production,time_vector,suptech,reno_gen_verify,
#                          this_tech_total_prod,all_gen_verify):
# 	"""
...
# Function #127
def update_primary_inv_effic(useful_pj_inputs_dict, get_src_gen, primary_inv_effic_dict, get_src_prim_ene,itpk):
	"""
...
# Function #128
def calculate_renewable_generation(renewable_key_list, total_production, primary_inv_effic_dict,time_vector):
	"""
...
# Function #130
def calculate_non_electricity_bio_demand(dict_energy_demand_by_fuel,time_vector):
	"""
...
# Function #131
def calculate_fuel_use_pp(fuel_use_electricity,time_vector):
	"""
...
def get_local_export_data(query_dict, time_vector, case, reg=None, con=None, key=None, tech=None):
	...
	try:
		...
		local_export_data = [0 for _ in range(len(time_vector))]
...
def bulac_test(base_inputs, dict_database, fut_id, this_hypercube, df_exp):
    print(fut_id)
    
def bulac_engine(base_inputs, dict_database, fut_id, this_hypercube, df_exp,
               flags_sectors, params_dict):

    ##############################################################################
    # SIMULATION: Implement the equations designed in "model_design" #
    print('Simulating future: ' + str(fut_id))
    
    # Extract the hypercube row:
    this_hypercube_row = this_hypercube[fut_id - 1]
    
    # 1) GDP
    mult_gdp = this_hypercube_row.item(0)
    min_val_gdp = df_exp['Min. Reduction'].tolist()[0]
    max_val_gdp = df_exp['Max. Reduction'].tolist()[0]
    val_gdp = stats.uniform.ppf(mult_gdp, min_val_gdp,
                                      max_val_gdp - min_val_gdp)
    sets_gdp = df_exp['Fuels'].tolist()[0].split(' ; ')
    # gdp_dict
    
    # 2) Elasticity of demand (passenger)
    mult_edempas = this_hypercube_row.item(1)
    min_val_edempas = df_exp['Min. Reduction'].tolist()[1]
    max_val_edempas = df_exp['Max. Reduction'].tolist()[1]
    val_edempas = stats.uniform.ppf(mult_edempas, min_val_edempas,
                                          max_val_edempas - min_val_edempas)
    sets_edempas = df_exp['Fuels'].tolist()[1].split(' ; ')
    # projtype_ela_pas
    
    # 3) Elasticity of demand (freight)
    mult_edemfre = this_hypercube_row.item(2)
    min_val_edemfre = df_exp['Min. Reduction'].tolist()[2]
    max_val_edemfre = df_exp['Max. Reduction'].tolist()[2]
    val_edemfre = stats.uniform.ppf(mult_edemfre, min_val_edemfre,
                                          max_val_edemfre - min_val_edemfre)
    sets_edemfre = df_exp['Fuels'].tolist()[2].split(' ; ')
    # projtype_ela_fre
       
    # 4) Fuel costs
    mult_fuelcosts = this_hypercube_row.item(3)
    min_val_fuelcosts = df_exp['Min. Reduction'].tolist()[3]
    max_val_fuelcosts = df_exp['Max. Reduction'].tolist()[3]
    val_fuelcosts = stats.uniform.ppf(mult_fuelcosts, min_val_fuelcosts,
                                            max_val_fuelcosts - min_val_fuelcosts)
    sets_fuelcosts = df_exp['Fuels'].tolist()[3].split(' ; ')
    # list_var_cost (SELECT THE TECHS)
    
    # 5) Electricity and H2 costs
    mult_e_h2_costs = this_hypercube_row.item(4)
    min_val_e_h2_costs = df_exp['Min. Reduction'].tolist()[4]
    max_val_e_h2_costs = df_exp['Max. Reduction'].tolist()[4]
    val_e_h2_costs = stats.uniform.ppf(mult_e_h2_costs, min_val_e_h2_costs,
                                             max_val_e_h2_costs - min_val_e_h2_costs)
    sets_e_h2_costs = df_exp['Fuels'].tolist()[4].split(' ; ')
    # list_var_cost (SELECT THE TECHS)
    
    # 6) ZEV capital costs
    mult_zev_capcost = this_hypercube_row.item(5)
    min_val_zev_capcost = df_exp['Min. Reduction'].tolist()[5]
    max_val_zev_capcost = df_exp['Max. Reduction'].tolist()[5]
    val_zev_capcost = stats.uniform.ppf(mult_zev_capcost, min_val_zev_capcost,
                                              max_val_zev_capcost - min_val_zev_capcost)
    sets_zev_capcost = df_exp['Fuels'].tolist()[5].split(' ; ')
    # list_cap_cost (SELECT THE TECHS)
    
    # 7) Fuel consumption
    mult_fuelcons = this_hypercube_row.item(6)
    min_val_fuelcons = df_exp['Min. Reduction'].tolist()[6]
    max_val_fuelcons = df_exp['Max. Reduction'].tolist()[6]
    val_fuelcons = stats.uniform.ppf(mult_fuelcons, min_val_fuelcons,
                                           max_val_fuelcons - min_val_fuelcons)
    sets_fuelcons = df_exp['Fuels'].tolist()[6].split(' ; ')
    # dict_fuel_economy (SELECT THE TECHS)
    
    # 8) Electricity and H2 performance
    mult_e_h2_perf = this_hypercube_row.item(7)
    min_val_e_h2_perf = df_exp['Min. Reduction'].tolist()[7]
    max_val_e_h2_perf = df_exp['Max. Reduction'].tolist()[7]
    val_e_h2_perf = stats.uniform.ppf(mult_e_h2_perf, min_val_e_h2_perf,
                                            max_val_e_h2_perf - min_val_e_h2_perf)
    sets_e_h2_perf = df_exp['Fuels'].tolist()[7].split(' ; ')
    # dict_fuel_economy (SELECT THE TECHS)
    
    # 9) Mode shift (public transport)
    mult_mshft = this_hypercube_row.item(8)
    min_val_mshft = df_exp['Min. Reduction'].tolist()[8]
    max_val_mshft = df_exp['Max. Reduction'].tolist()[8]
    val_mshft = stats.uniform.ppf(mult_mshft, min_val_mshft,
                                        max_val_mshft - min_val_mshft)
    sets_mshft = df_exp['Fuels'].tolist()[8].split(' ; ')
    # list_mode_shift
    # list_non_motorized
    
    # 10) Electrification
    mult_elec = this_hypercube_row.item(9)
    min_val_elec = df_exp['Min. Reduction'].tolist()[9]
    max_val_elec = df_exp['Max. Reduction'].tolist()[9]
    val_elec = stats.uniform.ppf(mult_elec, min_val_elec,
                                       max_val_elec - min_val_elec)
    sets_elec = df_exp['Fuels'].tolist()[9].split(' ; ')
    # list_electrification
    # list_hydrogen
    ...
    # Unpack flags sectors
    overwrite_transport_model = flags_sectors[0]
    model_agro_and_waste = flags_sectors[1]
    model_rac  = flags_sectors[2]
    model_irote = flags_sectors[3]
                ...
                *dict_energy_demand* dictionaries to reflect the effect of the 
                ...
                if overwrite_transport_model:
                    ...
                    if 'endogenous' not in projtype_dem_pas:
                        ...
                        # Modify 1:
                        this_gdp_growth_vals_2 = interpolation_non_linear_final(
                            time_vector, this_gdp_growth_vals, val_gdp, ini_simu_yr)
                        this_gdp_pc_growth_vals_2 = interpolation_non_linear_final(
                            time_vector, this_gdp_pc_growth_vals, val_gdp, ini_simu_yr)
                        # Modify 2:
                        list_ela_pas_2 = interpolation_non_linear_final(
                            time_vector, list_ela_pas, val_edempas, ini_simu_yr)
                        ...
                        # pass_trn_dem, pass_trn_dem_2 = project_demand(time_vector, ini_simu_yr, projtype_dem_pas, mask_dem_pas, df_trn_data, sum_pass_trn_dem_by, this_gdp_growth_vals, this_gdp_pc_growth_vals, this_pop_growth_vals, list_ela_pas, 'pas')
                    ...
                    if 'endogenous' not in projtype_dem_fre:
                        ...
                        # Modify 3:
                        list_ela_fre_2 = interpolation_non_linear_final(
                            time_vector, list_ela_fre, val_edemfre, ini_simu_yr)
                        fre_trn_dem, fre_trn_dem_2 = project_demand(time_vector, ini_simu_yr, projtype_dem_fre, mask_dem_fre, df_trn_data, sum_fre_trn_dem_by, this_gdp_growth_vals, this_gdp_growth_vals_2, this_gdp_pc_growth_vals, this_gdp_pc_growth_vals_2, this_pop_growth_vals, list_ela_fre, list_ela_fre_2, 'fre')
                        # fre_trn_dem, fre_trn_dem_2 = project_demand(time_vector, ini_simu_yr, projtype_dem_fre, mask_dem_fre, df_trn_data, sum_fre_trn_dem_by, this_gdp_growth_vals, this_gdp_pc_growth_vals, this_pop_growth_vals, list_ela_fre, 'fre')
                    ...
                    # Modify 1, 2, 3:
                    # For different demands (GDP and elasticities),
                    # we must calculate a fleet adjustment factor per scenario.
                    mult_pass_fleet = [
                        pass_trn_dem_2[i]/v for i, v in enumerate(pass_trn_dem)]
                    mult_pass_fleet[0] = 1
                    mult_fre_fleet = [
                        fre_trn_dem_2[i]/v for i, v in enumerate(fre_trn_dem)]
                    mult_fre_fleet[0] = 1
                    ...
                    list_non_motorized, mask_non_motorized = extract_data_from_dataframe((df_trn_data['Parameter'] == 'Non-motorized transport'), time_vector, df_trn_data, this_country)
                    ...
                    for t in types_all:
                        ...
                        for af in fuels:
                            ...
                            list_perf_raw = deepcopy(add_fe_k)
                            if af in sets_e_h2_perf:
                                # Modify 8: must interpolate the fuel economy here
                                add_fe_k = interpolation_non_linear_final(
                                    time_vector, list_perf_raw, val_e_h2_perf, ini_simu_yr)
                            elif af in sets_fuelcons:
                                # Modify 7: must interpolate the fuel economy here
                                add_fe_k = interpolation_non_linear_final(
                                    time_vector, list_perf_raw, val_fuelcons, ini_simu_yr)
                            else:
                                print('#8 AND 7 - ', af, ' - undefined')
                    ...
                    for this_f in fuels:
                        ...
                        for y in range(len(time_vector)):
                            ...
                            for t in types_all:
                                # Modifying 1, 2, 3 below with "mult_fleet":
                                if t in list_fre_trn:
                                    mult_fleet = deepcopy(mult_fre_fleet)
                                elif t in list_pass_trn:
                                    mult_fleet = deepcopy(mult_pass_fleet)
                                else:
                                    print('#1, 2, 3 - ', t, ' - not defined.')
                                ...
                                this_fuel_con = apply_projection_overwrites(y, t, this_f, dict_proj_fleet, this_fleet_k, dict_gpkm_gtkm, this_fuel_con, dict_km, this_fe, dict_lf, dict_fleet_k, dict_fuel_con, mult_fleet)
                    ...
                    for t in types_all:
                        ...
                        for f in fuels:
                            ...
                            if f in sets_zev_capcost:
                                # Modify 6: modify the capital costs
                                list_cap_cost_raw = deepcopy(list_cap_cost)
                                list_cap_cost = interpolation_non_linear_final(
                                    time_vector, list_cap_cost_raw, val_zev_capcost, ini_simu_yr)
                            ...
                            list_var_cost_raw = deepcopy(list_var_cost)
                            if f in sets_fuelcosts:
                                # Modify 4: modify the variable costs
                                list_var_cost = interpolation_non_linear_final(
                                    time_vector, list_var_cost_raw, val_fuelcosts, ini_simu_yr)
                            if f in sets_e_h2_costs:
                                # Modify 5: modify the variable costs
                                list_var_cost = interpolation_non_linear_final(
                                    time_vector, list_var_cost_raw, val_e_h2_costs, ini_simu_yr)
                ...
                # ... here we already have a country's deSmands, now...
                ...
                loss_vector = create_loss_vector(df_param_related_11, time_vector, maxloss_projection, maxloss_baseyear_str, losses_share, ini_simu_yr)
                ...
                for tech in list_electric_sets_3:
                    ...
                    # print(type(d5_power_techs))
                    # print(d5_power_techs.head())
                ...
                for tech in list_electric_sets_3:
                    ...
                    if (this_tech_dneeg_projection == 'interpolate' and
                        ...
                        for y in range(len(time_vector)):
                            ...
                            new_energy_assign = calculate_new_energy_assignment(y, tech, this_tech_base_cap, list_tech_cf, res_energy_sum_1, new_req_energy, store_percent, res_energy_sum, this_tech_dneeg_projection)
                ...
                for tech in list_electric_sets_3_shuffle:
                    ...
                    if (this_tech_dneeg_projection == 'interpolate' and
                        ...
                        for y in range(len(time_vector)):
                            ...
                            if new_req_energy < 0:
                                ...
                                if not this_country == 'Uruguay':
                                    ...
                                    thermal_reductions, thermal_reductions_order, cf_by_tech = manage_thermal_reductions(
                                        ...
                                        this_country,time_vector,store_res_energy,forced_newcap_energy_by_tech)
                                else:
                                    new_req_energy = 0
                            else:
                                if not this_country == 'Uruguay':
                            ...
                            if not this_country == 'Uruguay' and tech == 'PP_Thermal_Natural Gas' and new_energy_assign > 0 and cf_by_tech[tech][y] < cf_ngas_max and y > 0:
                                cf_by_tech['PP_Thermal_Natural Gas'] = update_cf_for_thermal_gas('PP_Thermal_Natural Gas', y, new_energy_assign, res_energy, planned_energy, cf_by_tech, cf_ngas_max,time_vector,list_tech_cf,store_res_energy,forced_newcap_energy_by_tech)
                    ...
                    total_capacity[tech] = this_tech_total_cap
                    residual_capacity[tech] = this_tech_residual_cap
                    new_capacity[tech] = this_tech_new_cap
                    cap_new_unplanned[tech] = this_tech_new_cap_unplanned
                    cap_new_planned[tech] = this_tech_new_cap_planned
                    cap_phase_out[tech] = this_tech_phase_out_cap

                    total_production[tech] = this_tech_total_prod
                    new_production[tech] = this_tech_new_prod

                    ele_prod_share[tech] = this_tech_energy_dist
                    ele_endogenous[tech] = this_tech_total_endo
                    cap_accum[tech] = this_tech_accum_new_cap
                        ...
                        for y in range(len(time_vector)):
                            ...
                            if use_fuel in externality_fuels_list:  # ...store externalities                        
                                ...
                                add_value_globalwarming, add_value_localpollution, \
                                    list_externalities_globalwarming, list_externalities_localpollution \
                                        = calculate_externalities(y, use_fuel, list_use_fuel, externality_fuels_dict,list_externalities_globalwarming,list_externalities_localpollution)
        ...
        if model_irote:
            for r in range(len(regions_list)):
                ...
                for c in range(len(country_list)):
                    ...
                    #
                    reno_gen_list = calculate_renewable_generation(renewable_key_list, total_production, primary_inv_effic_dict,time_vector)
                    ...
                    non_ele_bio_list = calculate_non_electricity_bio_demand(dict_energy_demand_by_fuel,time_vector)
                    # Add fuel use by power plants:
                    fuel_use_pp_list = calculate_fuel_use_pp(fuel_use_electricity,time_vector)
