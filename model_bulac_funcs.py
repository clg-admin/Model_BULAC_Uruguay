...
#                 xp_coord_known.append(y)
#                 yp_coord_known.append(number_vector[0])
#             elif interp_indicator == 'ini':
#                 xp_coord_known.append(y)
#                 yp_coord_known.append(number_vector[0])

#     # Manejo de 'interme' para extrapolación
#     if interp_indicator == 'interme':
#         first_known_index = next((i for i, v in enumerate(number_vector) if v != ''), None)
#         last_known_index = len(number_vector) - 1 - next((i for i, v in enumerate(reversed(number_vector)) if v != ''), None)

#         if first_known_index is not None and last_known_index is not None:
#             first_known_value = number_vector[first_known_index]
#             last_known_value = number_vector[last_known_index]

#             # Extrapolación antes del primer punto conocido
#             for i in range(first_known_index - 1, -1, -1):
#                 xp_coord_known.insert(0, i)
#                 extrapolated_value = first_known_value - (first_known_index - i) * (last_known_value - first_known_value) / (last_known_index - first_known_index)
#                 if extrapolated_value < 0:  # Reemplazar valores negativos por 0.0
#                     extrapolated_value = 0.0
#                 yp_coord_known.insert(0, extrapolated_value)

#     # Eliminar duplicados en xp_coord_known y yp_coord_known
#     unique_coords = list(set(zip(xp_coord_known, yp_coord_known)))
#     unique_coords.sort(key=lambda x: x[0])  # Ordenar por los índices
#     xp_coord_known, yp_coord_known = zip(*unique_coords)
...
#         print("x_coord_tofill:", x_coord_tofill)
#         print("xp_coord_known:", xp_coord_known)
#         print("yp_coord_known:", yp_coord_known)
#         raise e

#     # Generar lista de valores interpolados
#     interpolated_values = []
#     for coord in range(len(time_vector)):
#         if coord in xp_coord_known:
#             value_index = xp_coord_known.index(coord)
...
#             yp_coord_known.append(number_vector[y])
#         else:
#             if interp_indicator in ['last', 'ini']:
#                 xp_coord_known.append(y)
#                 yp_coord_known.append(number_vector[0])

#     # Manejar extrapolación para 'interme'
#     if interp_indicator == 'interme':
#         first_known_index = next(i for i, v in enumerate(number_vector) if v != '')
#         last_known_index = len(number_vector) - 1 - next(i for i, v in enumerate(reversed(number_vector)) if v != '')
#         first_known_value = number_vector[first_known_index]
#         last_known_value = number_vector[last_known_index]

#         # Extrapolar valores antes del primer punto conocido
#         for i in range(first_known_index - 1, -1, -1):
#             extrapolated_value = first_known_value - (first_known_index - i) * (last_known_value - first_known_value) / (last_known_index - first_known_index)
#             if extrapolated_value < 0:  # Reemplazar valores negativos por 0.0
#                 extrapolated_value = 0.0
...
#     # Validar si hay datos para interpolar
#     y_coord_filled = []
#     if len(x_coord_tofill) > 0 and len(xp_coord_known) > 1:
#         try:
#             y_coord_filled = list(np.interp(x_coord_tofill, xp_coord_known, yp_coord_known))
#         except Exception as e:
...
#         elif coord in x_coord_tofill:
#             value_index = x_coord_tofill.index(coord)
#             interpolated_values.append(float(y_coord_filled[value_index]) if len(y_coord_filled) > value_index else None)
...
def fun_unpack_taxes(param, tech, fuel, df_tax_trn, time_vector, dict_km):
    '''
    ...
    return list_val, ref_param, mult_depr, tech_km
...
def compute_delta_for_technology(total_list, time_vector, list_life):
    """
    Calculate the year-over-year changes (delta) in technology data over time.

    This function computes the delta of a technology list based on the total list of values and 
    the specified lifetime. It adjusts the delta values considering the lifecycle of each technology 
    element in the list, providing a yearly change in the technology considering its lifetime.

    Parameters:
    - total_list (list): A list of total values for a technology over time.
    - time_vector (list): A list of time intervals (usually years).
    - list_life (list): A list of life duration for each technology element.

    Returns:
    - list: A list of delta values representing year-over-year changes.
    """
    delta_list = [0]
    for y in range(1, len(time_vector)):
...
def discounted_values(values_list, discount_rate):
    """
    Calculate the discounted values of a list of future values based on a given discount rate.

    This function applies discounting to a list of values, typically representing future cash flows or costs. 
    The discounting adjusts the future values to their present values considering the time value of money. 
    It's commonly used in financial analysis and investment decision-making.

    Parameters:
    - values_list (list): A list of future values to be discounted.
    - discount_rate (float): The discount rate to apply for the calculation.

    Returns:
    - list: A list of present values after applying the discount rate.
    """
